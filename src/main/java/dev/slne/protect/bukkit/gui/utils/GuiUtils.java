...
public class GuiUtils {
     ...
     */
    public static StaticPane getNavigation(ProtectionGui gui, int row) {
        ...
        if (gui.hasParent() && gui.getViewingPlayer().hasPermission("surf.shop.item.back-item")) {
            navigation.addItem(new GuiItem(ItemUtils.backItem(gui), event -> gui.showParent((Player) event.getWhoClicked())), 3, 0);
        }
