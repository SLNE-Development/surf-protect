...
class ProtectionUser(val uuid: UUID) {
    ...
    suspend fun startRegionCreation(
        ...
        errorHandler: (Component) -> Unit
    ): Boolean {
        val player = this.bukkitPlayer ?: return false

        when {
            isCreatingRegion -> {
                player.sendText { appendErrorPrefix(); error("Du befindest dich bereits im ProtectionMode.") }
                return false
            }

            !player.location.isInProtectionRegion() -> {
                player.sendText { appendErrorPrefix(); error("Du kannst hier keinen ProtectionMode starten.") }
                return false
            }
