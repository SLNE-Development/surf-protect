...
object ProtectionInfoDialog {
    fun createProtectionInfoDialog(
        ...
        val tpLocation = info.teleportLocation
        ...
        base {
            ...
            body {
                plainMessage {
                    variableKey("Größe: ")
                    variableValue("${info.area} Blöcke")
                }
                plainMessage {
                    variableKey("Entfernung: ")
                    if (tpLocation == null || tpLocation.world != viewer.world) {
                        variableValue("Unbekannt")
                    } else {
                        variableValue("${viewer.location.distance(tpLocation).toLong()} Blöcke")
                    }
                }
                plainMessage {
                    variableKey("Ort: ")
                    if (tpLocation == null) {
                        ...
                        variableValue("${tpLocation.blockX}, ${tpLocation.blockY}, ${tpLocation.blockZ} — ${tpLocation.world?.name ?: "Unbekannt"}")
                    }
                }
                plainMessage {
                    variableKey("Besitzer: ")
                    val ownerNames = ProtectionUtils.getOwnerNames(info.region)
                ...
                plainMessage {
                    ...
                    val memberNames = ProtectionUtils.getMemberNames(info.region)
        ...
        type {
            multiAction {
                if (tpLocation != null && viewer.hasPermission(ProtectPermissionRegistry.PROTECTION_TELEPORT)) {
                    action(teleportButton(tpLocation))
                }
